...
const ccVersion = '4.0'
const ccPrefix = 'licenses'
const ccType = 'by-nc-sa'
const ccURL = computed(() => {
  const ccLang = 'en'
  return `https://creativecommons.org/${ccPrefix}/${ccType}/${ccVersion}/deed.${ccLang}`
})
const rootData = useAggregateData()
const user = rootData!.user
const licenseHtml = computed(() => {
  return t('post.copyright.license_content', [
    `<a href="${
      ccURL.value
    }" target="_blank" rel="noopener" title="CC ${`${ccType.toUpperCase()} ${ccVersion}`} ">CC ${ccType.toUpperCase()}</a>`,
  ])
})
</script>
